...
// Canonical vocabulary (genres, moods, podcast categories) for spelling-tolerant matching.
const VOCAB = [
  ...catalog.genres.map((g) => g.name),
  ...catalog.moods.map((m) => m.name),
  ...Array.from(new Set(catalog.podcasts.map((p) => p.category))),
];
function correctTerm(v) {
  if (!v) return "";
  return fuzzy.bestMatch(v, VOCAB, (x) => x, 0.6) || v;
}
...
async function runTool(name, args, channel) {
  try {
    if (name === "discover") {
      const parts = [
        correctTerm(args.mood), args.activity, args.language,
        correctTerm(args.genre), args.artist, args.contentType,
      ].filter(Boolean);
