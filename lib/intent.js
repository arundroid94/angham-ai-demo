...
function buildAiResponse(intent, contentType) {
  ...
  const desc = [
    intent.mood ? intent.mood.toLowerCase() : "",
    intent.language ? cap(intent.language) : "",
    intent.genre ? intent.genre : "",
    intent.category ? intent.category.toLowerCase() : "",
  ].filter(Boolean).join(" ");
...
function parseCommand(query, state) {
  ...
  const isOpen = /\b(open|show me|go to|view|see)\b/.test(q);
  const isPlay = /\b(play|start|put on|listen to|resume)\b/.test(q);
